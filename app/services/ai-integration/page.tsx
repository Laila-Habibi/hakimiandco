import Link from "next/link";
import {
  ArrowRight,
  Bot,
  BrainCircuit,
  BriefcaseBusiness,
  Building2,
  ChartNoAxesCombined,
  Check,
  CircleCheckBig,
  Cloud,
  FileSearch,
  Gauge,
  Network,
  RefreshCw,
  Sparkles,
  Workflow,
} from "lucide-react";

const audiences = [
  {
    title: "Small Businesses",
    description:
      "Introduce practical AI tools that reduce repetitive work and improve everyday business processes without unnecessary complexity.",
    icon: BriefcaseBusiness,
  },
  {
    title: "Growing Companies",
    description:
      "Use automation, intelligent reporting and connected workflows to support increasing workloads and expanding operations.",
    icon: Building2,
  },
  {
    title: "Professional Teams",
    description:
      "Improve productivity, reporting and decision-making with AI solutions tailored to your team’s responsibilities and goals.",
    icon: Network,
  },
];

const aiServices = [
  {
    title: "Workflow Automation",
    description:
      "We identify repetitive business tasks and develop automated workflows that reduce manual work, improve consistency and help your team complete routine processes more efficiently.",
    icon: Workflow,
  },
  {
    title: "AI-Powered Financial Analysis",
    description:
      "We help integrate AI tools that review financial data, identify trends and support faster analysis. These solutions can improve visibility and help decision-makers understand business performance more clearly.",
    icon: ChartNoAxesCombined,
  },
  {
    title: "Intelligent Document Processing",
    description:
      "AI can help organize, extract and classify information from invoices, receipts, forms and other business documents. We design solutions that reduce data-entry work and improve document-handling efficiency.",
    icon: FileSearch,
  },
  {
    title: "Custom AI Assistants",
    description:
      "We develop tailored AI assistants that help teams answer internal questions, summarize information, prepare drafts and access approved business knowledge more efficiently.",
    icon: Bot,
  },
  {
    title: "Reporting and Business Intelligence",
    description:
      "We connect AI capabilities with dashboards and reporting tools to make financial and operational information easier to access, understand and use in everyday decision-making.",
    icon: Gauge,
  },
  {
    title: "System and Process Integration",
    description:
      "We help connect AI tools with your existing software, data sources and business processes so information can move more smoothly across your organization.",
    icon: Cloud,
  },
];

const approachSteps = [
  {
    title: "Discover",
    description:
      "We learn about your current processes, challenges, software and business priorities.",
  },
  {
    title: "Identify",
    description:
      "We determine where AI and automation can provide practical value without adding unnecessary complexity.",
  },
  {
    title: "Implement",
    description:
      "We design and integrate solutions that fit your workflow, technology and team requirements.",
  },
  {
    title: "Improve",
    description:
      "We review results, refine the solution and help your team use the new tools effectively.",
  },
];

const aiUseCases = [
  "Invoice and receipt data extraction",
  "Automated document classification",
  "Financial report summarization",
  "Internal knowledge assistants",
  "Recurring task and workflow automation",
  "Customer inquiry support",
  "Business performance analysis",
  "Dashboard and reporting automation",
  "Email and document drafting support",
  "Data validation and error detection",
  "Process monitoring and alerts",
  "Integration with existing business systems",
];

const benefits = [
  {
    title: "Save Time",
    description:
      "Reduce repetitive administrative work and allow your team to focus on higher-value responsibilities.",
    icon: RefreshCw,
  },
  {
    title: "Improve Accuracy",
    description:
      "Use consistent automated processes to reduce avoidable errors in routine business tasks.",
    icon: CircleCheckBig,
  },
  {
    title: "Gain Better Insight",
    description:
      "Turn financial and operational data into clearer information that supports faster decisions.",
    icon: BrainCircuit,
  },
  {
    title: "Scale Efficiently",
    description:
      "Support growing workloads with systems and workflows that do not rely entirely on manual effort.",
    icon: Sparkles,
  },
];

export default function AIIntegrationPage() {
  return (
    <main className="overflow-hidden bg-white text-[#554b44]">
      <AIIntegrationHero />
      <WhoWeHelp />
      <AIServices />
      <OurApproach />
      <CommonAIUseCases />
      <Benefits />
      <AIIntegrationCTA />
    </main>
  );
}

function AIIntegrationHero() {
  return (
    <section className="relative overflow-hidden border-b border-[#80ab7e]/15 bg-[#f8f6ef] px-6 py-16 sm:px-10 lg:py-20">
      <div className="pointer-events-none absolute -right-24 -top-24 h-80 w-80 rounded-full bg-[#80ab7e]/10 blur-3xl" />

      <div className="pointer-events-none absolute -bottom-20 left-10 h-64 w-64 rounded-full bg-[#ffdb11]/10 blur-3xl" />

      <div className="relative mx-auto max-w-7xl">
        <div className="flex flex-wrap items-center gap-2 text-sm font-medium">
          <Link
            href="/"
            className="text-[var(--primary-green)] transition hover:text-[var(--primary-brown)]"
          >
            Home
          </Link>

          <span className="text-[#80ab7e]">›</span>

          <span
            
            className="text-[var(--primary-green)] transition hover:text-[var(--primary-brown)]"
          >
            Services
          </span>

          <span className="text-[#80ab7e]">›</span>

          <span className="text-[var(--primary-brown)]">
            AI Integration
          </span>
        </div>

        <div className="mt-10 max-w-4xl">
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#80ab7e]">
            Practical AI for Business
          </p>

          <h1 className="mt-4 font-serif text-4xl font-semibold leading-tight text-[var(--primary-brown)] sm:text-5xl">
            AI Integration
          </h1>

          <h2 className="mt-3 font-serif text-2xl leading-tight text-[var(--primary-green)] sm:text-3xl">
            Smarter Workflows. Better Insights. Greater Efficiency.
          </h2>

          <div className="mt-6 h-[3px] w-16 rounded-full bg-[var(--primary-golden)]" />

          <p className="mt-7 max-w-4xl text-base leading-8 text-[#5c5048]">
            Artificial intelligence can help businesses reduce repetitive work,
            improve reporting and make better use of financial and operational
            data. Hakimi &amp; Co. helps organizations identify practical AI
            opportunities and integrate solutions that support real business
            needs without adding unnecessary complexity.
          </p>
        </div>
      </div>
    </section>
  );
}

function WhoWeHelp() {
  return (
    <section className="px-6 py-20 sm:px-10">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          eyebrow="Who We Help"
          title="AI Solutions Designed Around Your Business"
          align="left"
        />

        <div className="mt-12 grid gap-8 lg:grid-cols-3">
          {audiences.map((audience, index) => {
            const Icon = audience.icon;

            return (
              <article
                key={audience.title}
                className={`group px-6 py-3 ${
                  index !== audiences.length - 1
                    ? "lg:border-r lg:border-[#854d1b]/15"
                    : ""
                }`}
              >
                <div className="flex items-start gap-5">
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-[#80ab7e]/12 text-[var(--primary-green)] transition duration-300 group-hover:-translate-y-1 group-hover:bg-[#80ab7e]/20">
                    <Icon size={31} strokeWidth={1.6} />
                  </div>

                  <div>
                    <h3 className="font-serif text-2xl text-[var(--primary-brown)]">
                      {audience.title}
                    </h3>

                    <p className="mt-3 text-sm leading-7 text-[#5c5048]">
                      {audience.description}
                    </p>
                  </div>
                </div>
              </article>
            );
          })}
        </div>
      </div>
    </section>
  );
}

function AIServices() {
  return (
    <section className="bg-[#fbfaf6] px-6 py-20 sm:px-10 lg:py-24">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          eyebrow="Our AI Integration Services"
          title="Practical Technology That Supports Real Work"
          align="left"
        />

        <div className="mt-10 divide-y divide-[#854d1b]/12">
          {aiServices.map((service) => {
            const Icon = service.icon;

            return (
              <article
                key={service.title}
                className="group grid gap-6 py-8 sm:grid-cols-[72px_1fr]"
              >
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#80ab7e]/12 text-[var(--primary-green)] transition duration-300 group-hover:scale-105 group-hover:bg-[#80ab7e]/20">
                  <Icon size={31} strokeWidth={1.6} />
                </div>

                <div>
                  <h3 className="font-serif text-2xl text-[var(--primary-brown)]">
                    {service.title}
                  </h3>

                  <p className="mt-3 max-w-5xl text-sm leading-7 text-[#5c5048]">
                    {service.description}
                  </p>
                </div>
              </article>
            );
          })}
        </div>
      </div>
    </section>
  );
}

function OurApproach() {
  return (
    <section className="px-6 py-20 sm:px-10">
      <div className="mx-auto grid max-w-7xl gap-12 rounded-3xl border border-[#80ab7e]/20 bg-[#f7f4e9] px-7 py-10 sm:px-10 lg:grid-cols-[0.8fr_1.2fr] lg:px-12 lg:py-14">
        <div>
          <p className="text-sm font-bold uppercase tracking-[0.18em] text-[#80ab7e]">
            Our Approach
          </p>

          <h2 className="mt-3 font-serif text-4xl leading-tight text-[var(--primary-brown)]">
            Practical. Secure.
            <span className="text-[var(--primary-green)]"> Purposeful.</span>
          </h2>

          <div className="mt-5 h-[3px] w-14 rounded-full bg-[var(--primary-golden)]" />

          <p className="mt-6 text-sm leading-7 text-[#5c5048]">
            We focus on AI solutions that solve a clear business problem. Our
            approach begins with your workflow and goals, not the technology
            itself. We then design practical integrations that are manageable,
            useful and aligned with how your team works.
          </p>
        </div>

        <div className="space-y-5 lg:border-l lg:border-[#854d1b]/15 lg:pl-10">
          {approachSteps.map((step) => (
            <div key={step.title} className="flex items-start gap-4">
              <div className="mt-1 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--primary-green)] text-white">
                <Check size={14} strokeWidth={2.5} />
              </div>

              <p className="text-sm leading-7 text-[#5c5048]">
                <strong className="text-[var(--primary-brown)]">
                  {step.title}:
                </strong>{" "}
                {step.description}
              </p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}

function CommonAIUseCases() {
  return (
    <section className="px-6 pb-20 sm:px-10">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          eyebrow="Common AI Use Cases"
          title="Business Processes We Can Help Improve"
          align="left"
        />

        <div className="mt-10 grid gap-x-10 gap-y-4 rounded-2xl bg-[#f7f4e9] px-7 py-8 sm:grid-cols-2">
          {aiUseCases.map((useCase) => (
            <div key={useCase} className="flex items-start gap-3">
              <span className="mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[var(--primary-green)] text-white">
                <Check size={12} strokeWidth={2.5} />
              </span>

              <p className="text-sm leading-6 text-[#4d3a2e]">{useCase}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}

function Benefits() {
  return (
    <section className="bg-[#fbfaf6] px-6 py-20 sm:px-10">
      <div className="mx-auto max-w-7xl">
        <SectionHeading
          eyebrow="Why Integrate AI"
          title="Make Everyday Work More Efficient"
          align="left"
        />

        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-4 lg:gap-0">
          {benefits.map((benefit, index) => {
            const Icon = benefit.icon;

            return (
              <article
                key={benefit.title}
                className={`px-6 ${
                  index !== benefits.length - 1
                    ? "lg:border-r lg:border-[#854d1b]/15"
                    : ""
                }`}
              >
                <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#80ab7e]/12 text-[var(--primary-green)]">
                  <Icon size={31} strokeWidth={1.6} />
                </div>

                <h3 className="mt-5 font-serif text-xl text-[var(--primary-brown)]">
                  {benefit.title}
                </h3>

                <p className="mt-3 text-sm leading-7 text-[#5c5048]">
                  {benefit.description}
                </p>
              </article>
            );
          })}
        </div>
      </div>
    </section>
  );
}

function AIIntegrationCTA() {
  return (
    <section className="px-6 py-20 sm:px-10">
      <div className="relative mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 overflow-hidden rounded-2xl bg-[var(--primary-green)] px-8 py-10 text-white sm:px-12 lg:flex-row lg:items-center">
        <div className="pointer-events-none absolute -left-20 -top-24 h-64 w-64 rounded-full border border-white/10" />

        <div className="pointer-events-none absolute -bottom-28 right-16 h-72 w-72 rounded-full border border-white/10" />

        <div className="relative max-w-2xl">
          <h2 className="font-serif text-3xl font-semibold sm:text-4xl">
            Explore What AI Can Do for Your Business
          </h2>

          <p className="mt-3 max-w-xl text-sm leading-7 text-white/80">
            Speak with our team about workflow automation, intelligent
            reporting, document processing and custom AI solutions designed
            around your business needs.
          </p>
        </div>

        <Link
          href="/contact"
          className="group relative inline-flex shrink-0 items-center gap-3 rounded-lg bg-white px-6 py-3.5 text-sm font-semibold text-[var(--primary-green)] transition hover:-translate-y-1 hover:shadow-xl"
        >
          Book a Consultation

          <ArrowRight
            size={17}
            className="transition-transform duration-300 group-hover:translate-x-1"
          />
        </Link>
      </div>
    </section>
  );
}

function SectionHeading({
  eyebrow,
  title,
  description,
  align = "center",
  showUnderline = true,
}: {
  eyebrow: string;
  title: string;
  description?: string;
  align?: "left" | "center";
  showUnderline?: boolean;
}) {
  const isLeft = align === "left";

  return (
    <div
      className={`max-w-3xl ${
        isLeft ? "text-left" : "mx-auto text-center"
      }`}
    >
      <p className="text-sm font-bold uppercase tracking-[0.17em] text-[#80ab7e]">
        {eyebrow}
      </p>

      <h2 className="mt-3 font-serif text-4xl font-semibold leading-tight text-[var(--primary-brown)]">
        {title}
      </h2>

      {showUnderline && (
        <div
          className={`mt-4 h-[3px] w-14 rounded-full bg-[var(--primary-golden)] ${
            isLeft ? "" : "mx-auto"
          }`}
        />
      )}

      {description && (
        <p
          className={`text-sm leading-7 text-[#554b44] ${
            showUnderline ? "mt-5" : "mt-4"
          }`}
        >
          {description}
        </p>
      )}
    </div>
  );
}